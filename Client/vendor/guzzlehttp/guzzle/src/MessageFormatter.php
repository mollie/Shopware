...
class MessageFormatter
{
     ...
     * @return string
     */
    public function format(\_PhpScoperd1ad3ba9842f\Psr\Http\Message\RequestInterface $request, \_PhpScoperd1ad3ba9842f\Psr\Http\Message\ResponseInterface $response = null, \Exception $error = null)
    {
        $cache = [];
        return \preg_replace_callback('/{\\s*([A-Za-z_\\-\\.0-9]+)\\s*}/', function (array $matches) use($request, $response, $error, &$cache) {
            ...
            switch ($matches[1]) {
                case 'request':
                    $result = \_PhpScoperd1ad3ba9842f\GuzzleHttp\Psr7\str($request);
                    break;
                case 'response':
                    $result = $response ? \_PhpScoperd1ad3ba9842f\GuzzleHttp\Psr7\str($response) : '';
                    break;
                case 'req_headers':
    ...
    private function headers(\_PhpScoperd1ad3ba9842f\Psr\Http\Message\MessageInterface $message)
    {
        $result = '';
