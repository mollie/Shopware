<?php
namespace GuzzleHttpV6\Promise\Tests;

use GuzzleHttpV6\Promise as P;
use GuzzleHttpV6\Promise\AggregateException;
...
use GuzzleHttpV6\Promise\RejectionException;
use GuzzleHttpV6\Promise\TaskQueue;
...
class FunctionsTest extends TestCase
{
    ...
    {
        $p = \GuzzleHttpV6\Promise\promise_for('foo');
    ...
    {
        $p = new Promise();
        $this->assertSame($p, \GuzzleHttpV6\Promise\promise_for($p));
    }

    public function testReturnsPromiseForThennable()
    {
        $p = new Thennable();
        $wrapped = \GuzzleHttpV6\Promise\promise_for($p);
    ...
    {
        $p = \GuzzleHttpV6\Promise\rejection_for('fail');
    ...
    {
        $a = new Promise();
        $b = \GuzzleHttpV6\Promise\rejection_for($a);
        $this->assertSame($a, $b);
    }
    ...
    public function testWaitsOnAllPromisesIntoArray()
    {
        ...
        $results = \GuzzleHttpV6\Promise\inspect_all([$a, $b, $c]);
        $this->assertEquals([
    ...
    public function testAllAggregatesSortedArray()
    {
        ...
        $b = new Promise();
        $c = new Promise();
        $d = \GuzzleHttpV6\Promise\all([$a, $b, $c]);
        $b->resolve('b');
        $a->resolve('a');
        $c->resolve('c');
        $d->then(
    ...
    public function testPromisesDynamicallyAddedToStack()
    {
        ...
        $result = \GuzzleHttpV6\Promise\all($promises, true)->wait();
    ...
    public function testSomeAggregatesSortedArrayWithMax()
    {
        ...
        $b = new Promise();
        $c = new Promise();
        $d = \GuzzleHttpV6\Promise\some(2, [$a, $b, $c]);
        $b->resolve('b');
        $c->resolve('c');
        $a->resolve('a');
    ...
    public function testSomeRejectsWhenTooManyRejections()
    {
        $a = new Promise();
        $b = new Promise();
        $d = \GuzzleHttpV6\Promise\some(2, [$a, $b]);
        $a->reject('bad');
        $b->resolve('good');
        P\queue()->run();
    ...
    public function testThrowsIfResolvedWithoutCountTotalResults()
    {
        $a = new Promise();
        $b = new Promise();
        $d = \GuzzleHttpV6\Promise\some(3, [$a, $b]);
        $a->resolve('a');
        $b->resolve('b');
        $d->wait();
    }
    ...
    public function testAnyReturnsFirstMatch()
    {
        $a = new Promise();
        $b = new Promise();
        $c = \GuzzleHttpV6\Promise\any([$a, $b]);
        $b->resolve('b');
        $a->resolve('a');
        //P\queue()->run();
    ...
    public function testSettleFulfillsWithFulfilledAndRejected()
    {
        ...
        $b = new Promise();
        $c = new Promise();
        $d = \GuzzleHttpV6\Promise\settle([$a, $b, $c]);
        $b->resolve('b');
        $c->resolve('c');
        $a->reject('a');
        P\queue()->run();
    ...
    public function testCanInspectFulfilledPromise()
    {
        ...
        $this->assertEquals([
            'state' => 'fulfilled',
            'value' => 'foo'
        ], \GuzzleHttpV6\Promise\inspect($p));
    }
    ...
    {
        $p = new Promise();
        $aggregate = \GuzzleHttpV6\Promise\each_limit($p, 2);
        $p->resolve('a');
        P\queue()->run();
    ...
    public function testEachLimitAllRejectsOnFailure()
    {
        ...
        $aggregate = \GuzzleHttpV6\Promise\each_limit_all($p, 2);
        P\queue()->run();
        $this->assertEquals(PromiseInterface::REJECTED, $aggregate->getState());
        $result = \GuzzleHttpV6\Promise\inspect($aggregate);
    ...
    public function testIterForReturnsIterator()
    {
        ...
        $this->assertSame($iter, \GuzzleHttpV6\Promise\iter_for($iter));
    }
